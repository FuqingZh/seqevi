...
> Date: 2026-07-29
> Supersedes: `20260720-v1.0-storage-deployment-architecture.md`
...
The existing node4 Apache service provides a dedicated plain HTTP listener on
the node4 private address, requires the observed cluster source ranges, and
proxies only to `http://127.0.0.1:18081`. PostgreSQL stays on loopback and the
artifact directory is never served by the proxy.

The allowed cluster CIDRs are the complete trust boundary for this minimal
increment. It introduces no client authentication API, credential file,
certificate, CA distribution, or secret-injection contract. This is acceptable
only while every workload able to originate traffic from an allowed CIDR is a
trusted Store writer.
...
must be protected by an explicit deployment trust boundary. The minimal node4
deployment trusts only its configured private cluster CIDRs; deployments that
include untrusted workloads must add an authenticated transport boundary.
PostgreSQL credentials remain available only to the service.

The API still validates paths, sizes, media types, digests, and schema. Those
integrity checks do not authenticate clients: any allowed client can upload
artifacts and commit immutable evidence.
