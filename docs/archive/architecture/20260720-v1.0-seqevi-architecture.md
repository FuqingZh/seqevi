...
> Result-delivery note: the 0.1.0 directory package described below is
> superseded by the approved v1.1 DuckDB result contract in
> `20260804-v1.1-result-consumption-contract.md`. The Store and adapter
> boundaries in this historical architecture remain applicable.
...
only for missing sequences, and materializes an adapter-specific DuckDB result.
...
flowchart LR
    ...
    G --> H
    H --> I["Materialize adapter-specific DuckDB result"]
```
...
FASTA is validated into temporary staging before any Store access. Subsequent
lookup, tool, and commit work uses bounded batches. Native tool outputs remain
files; normalized artifacts remain bounded Parquet parts, and the final DuckDB
result uses file-backed artifact scans and SequenceID joins.
...
V1 exposes three command surfaces:
...
- `annotate` parses input, resolves cache hits, runs one adapter for misses,
  commits valid evidence, and writes the output DuckDB result.
- `resource verify` initializes or fully audits the SeqEvi lock for one exact
  user-supplied annotation database.
...
See the [current sequence and evidence contract](20260810-v1.2-sequence-evidence-contract.md)
...
- keep every adapter in its own schema and output result.

SeqEvi does not create one universal annotation table.

Artifact transfer uses file references or chunk streams with exact digest and
size verification. The local Store returns immutable CAS paths. Shared clients
batch metadata fetches and download each unique digest into a client-owned
temporary artifact directory, avoiding both N+1 metadata requests and repeated
...
| Result delivery | DuckDB and native relations | Single self-describing queryable file |
