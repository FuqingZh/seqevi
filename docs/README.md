...
| shared Store claim contention hardening | approved two-PR plan: PR A delivers the fail-closed 0004 maintenance preflight/runbook and an end-to-end ClaimSession annotate path before removing unreleased per-key routes; PR B adds lease-loss process-group cancellation and final acceptance; implementation not started | [active ClaimSession plan](implementation-plan/20260812-v1.0-claim-session-and-tool-cancellation-implementation-plan.md), [superseded per-key hardening plan](implementation-plan/20260811-v1.0-shared-store-claim-contention-hardening.md) |
