...
authentication is configured, the Store URL must use HTTPS and the client
reads exactly two non-empty UTF-8
...
Apache removes the deployment `Authorization` header after authenticating the
request and before proxying it over loopback. The reusable credential therefore
terminates at the TLS ingress boundary.
