...
The 0.3.2 candidate source tree uses strict protein sequence identity, the single-host
SQLite/POSIX Store, the external tool runner, exact cache-miss orchestration,
and atomic DuckDB result materialization.

SeqEvi 0.3.2 provides managed setup for dbCAN only. The `eggnog` and
